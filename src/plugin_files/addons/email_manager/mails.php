	...
	public static function add_user_course( $user_id, $course_id ) {
		...
		if ( ! empty( $course_id ) ) {
			...
			if ( ! empty( $post_author ) ) {
				...
				if ( ! empty( $post_author_info->user_email ) ) { // ChinDevs changed from $co_instructor_info existance check to $post_author_info
		...
		// ChinDevs code to not send enrollment email to instructor
		if (!in_array($user['email'], $authors)) {
			...
			//ChinDevs code to send additional email to Instructors of the course
			$message = sprintf(
				/* translators: %s Course Title  %s Login*/
				esc_html__( 'New User %s enrolled in course %s.', 'masterstudy-lms-learning-management-system' ),
				$login,
				$course_title
			);
			...
			if (count($authors) >= 1) {
				...
				self::send_email( 'New User enrolled in Course.', $message, $main_author, $authors, 'stm_lms_user_enrolled_in_course', compact( 'course_title', 'login' ) );
			}
			//End ChinDevs code
		}
		self::remove_wp_mail_text_html();
