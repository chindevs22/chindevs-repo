...
// 			$content = '<div class="' . implode( ' ', $content_class ) . '">' . apply_filters( 'the_content', get_the_content() ) . '</div>';
			...
			global  $product_ids, $post_id;
			$post_id = $post->ID;
			if (Woocommerce_Pay_Per_Post_Helper::has_access($post->ID)) {
// 				$content = '<div class="' . implode( ' ', $content_class ) . '"> All content for you my dear! </div>';
				$content = '<div class="' . implode( ' ', $content_class ) . '">' . apply_filters( 'the_content', get_the_content() ) . '</div>';
			} else {
				$product_ids = Woocommerce_Pay_Per_Post_Helper::get_product_ids_by_post_id( $post->ID );
// 				$content = '<div class="' . implode( ' ', $content_class ) . '"> ' . $product_ids .  'No content for you my dear!' . ' </div>';
				$content = '<div class="' . implode( ' ', $content_class ) . '">' . Woocommerce_Pay_Per_Post_Helper::get_no_access_content() . '</div>';
			}
