...
function create_course_data() {
    ...
    read_csv("lesson_materials.csv", "lesson");
...
function progress_users_quiz_from_csv($progressData) {
    ...
    echo "<br> inside user quiz for " . $wp_user_id . "<br>";
...
function progress_user_lessons($wp_course_id, $wp_quiz_id, $wp_user_id) {
	global $wpdb;
    echo "<br> inside progress user lessons <br>";
    $curriculum_string = get_post_meta($wp_course_id, 'curriculum', true);
    echo "<br>Curriculum String: " . $curriculum_string . "<br>";
	echo "Quiz ID: " . $wp_quiz_id . "<br>";
    ...
    while (!$isLeft || !$isRight) {
       ...
       if(intval($ca[$indexLeft]) == 0) {
            echo "The lefter quiz " . $ca[$indexLeft];
            $isLeft = true;
       } else {
...
function progress_users_answers_from_csv($answerData) {
    echo "inside user answers users <br>";
...
function enrol_users_from_csv($enrolData) {
	...
	echo "ENROL USER ID: " . $userMGMLtoWP[$enrolData['user_id']] . "<br>";
	echo "COURSE: " . $courseMGMLtoWP[$enrolData['course_id']] . "<br>";
	...
	echo "POST META FOR STUDENTS " . $students;
...
//     echo "setting term for " . $sub_cat  . " and " . $parent_cat . " and " . $productData['title'] . "<br>";
	...
	foreach($extra_attributes as $attr) {
		...
		$value = trim($attr['value'], " \x3A");
		echo "trimmed key " . $key . "<br>";
		$formattedAttr = "" . $key . ": " . $value;

		echo "formatted attr: " . $formattedAttr . "<br>";
