...
function read_csv() {
	$fileName = '/home/freewaydns-dev108/cd-test-docs/test_codo_users.csv';
	$file = fopen($fileName, 'r');
	$headerFlag = true;
 	print "going to read CSV";
	while (($line = fgetcsv($file)) !== FALSE) {
		if ($headerFlag) { $headerFlag = false; continue; }
		print "inside createuser";
		create_user_from_csv($line[0], "FAKE PASS", $line[2]);
	}
	fclose($file);
	print "done reading CSV";
}

function create_user_from_csv($username, $password, $email) {
	print "inside createuser";
	...
	if ( !username_exists($username) && !email_exists($email) ) {
		...
		print "already created this user";
	}
}

function user_testermeta() {
	print ("teste!");
  update_user_meta(12, 'shipping_address_1', "Sample Shipping Address");
	print_r (get_user_meta(12));
}

add_shortcode( 'test', 'read_csv' );
add_shortcode( 'test2', 'user_testermeta');